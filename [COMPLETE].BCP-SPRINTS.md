...
**Estado del documento:** ✅ COMPLETO — Sprints 0–9.4 ejecutados y cerrados.
...
Este documento es el plan de trabajo concreto. Cada sprint tiene tareas, criterios de aceptación técnicos y resultado esperado. Lee la `[COMPLETE].BCP-SPECIFICATION.md` para entender el *por qué* de cada decisión.
...
- [x] `docs/[CURRENT].OPERATIONS.md`: guía de operaciones para el comprador (levantar, escalar, monitorear)
- [x] `docs/[CURRENT].PROVIDERS.md`: cómo agregar un nuevo Provider (paso a paso con código de ejemplo)
...
- La documentación `[CURRENT].PROVIDERS.md` permite implementar un Provider nuevo sin consultar el código existente
...
**Tests:** Los tests se escriben junto con el código. No hay "sprint de testing" separado. Un feature sin test no está terminado (ver DoD en `[COMPLETE].BCP-SPECIFICATION.md`).
